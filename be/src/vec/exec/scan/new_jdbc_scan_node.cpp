...
Status NewJdbcScanNode::_process_rf_exprs() {
    ...
    for (auto expr : _rf_vexpr_set) {
        if (VRuntimeFilterWrapper* runtime_filter =
            ...
            _convert_rf_expr_to_sql_str(expr, filters);
        }
    }
    if (!filters.empty()) {
...
Status NewJdbcScanNode::_convert_rf_expr_to_sql_str(const vectorized::VExprSPtr& expr,
    ...
    if (node_type == TExprNodeType::IN_PRED) {
        ...
        auto data_type = slot_expr->type().type;
        auto literal = static_cast<const vectorized::VLiteral*>(filter_impl->get_child(1).get());
...
std::string NewJdbcScanNode::_convert_value_data(PrimitiveType type, const void* value) {
    ...
    case TYPE_DATETIME: {
        ...
        int len = reinterpret_cast<const vectorized::VecDateTimeValue*>(value)->to_buffer(
                convert_buffer);
    ...
    case TYPE_DATEV2: {
        char convert_buffer[64];
        int len =
                reinterpret_cast<const vectorized::DateV2Value<vectorized::DateV2ValueType>*>(value)
                        ->to_buffer(convert_buffer);
        res = fmt::format("'{}'", std::string(convert_buffer, len));
        break;
    }
    case TYPE_DATETIMEV2: {
        char convert_buffer[64];
        int len = reinterpret_cast<const vectorized::DateV2Value<vectorized::DateTimeV2ValueType>*>(
                          value)
                          ->to_buffer(convert_buffer);
        res = fmt::format("'{}'", std::string(convert_buffer, len));
        break;
